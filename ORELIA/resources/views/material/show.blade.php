      ...
      <div class="card-body">
        ...
        <div class="row mb-3">
          <div class="col-md-6">
            ...
            <p class="text-muted">{{ $view_data['material']->created_at }}</p>
          </div>
          <div class="col-md-6">
            <label class="form-label fw-bold">Updated:</label>
            <p class="text-muted">{{ $view_data['material']->updated_at }}</p>
          </div>
        </div>

        <div class="d-flex gap-2">
          <a href="{{ route('material.list') }}" class="btn btn-secondary">Back</a>
          <form method="POST" action="{{ route('material.destroy', $view_data['material']->get_id()) }}" style="display:inline;">
            @csrf
            @method('DELETE')
